...
define ( 'WP_SOCIAL_URL',  WP_PLUGIN_URL . '/' . end( explode( DIRECTORY_SEPARATOR, dirname( __FILE__ ) ) ) );
...
function sc_social_connect_process_login( $is_ajax = false ){
	...
	if ( $user_id ) {
		...
		if ( $user_id && is_integer( $user_id ) ) {
			update_user_meta( $user_id, $sc_provider_identity_key, $sc_provider_identity );
			update_user_meta($user_id, 'gender', $sc_gender);
		}
		else {
			print_r($userdata);
			die();
